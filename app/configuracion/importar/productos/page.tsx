...
export default function ImportarProductosPage() {
  ...
  const handleDescargarPlantilla = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/plantilla-productos')
      if (!response.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPreview(data)
      setShowPreview(true)
      setSuccess(`Se encontraron ${data.length} productos para importar. Por favor revisa los datos antes de confirmar.`)
    } catch (err: any) {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError('Error al importar productos: ' + (result.error || 'Error desconocido'))
        setLoading(false)
        return
      }
      ...
      if (importados > 0) {
        ...
        setError(`❌ Importación fallida: No se importaron productos.\n\n${informe}`)
      }
      
      setShowPreview(false)
      setPreview([])
    } catch (err: any) {
      setError('Error al importar productos: ' + (err.message || 'Error desconocido'))
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-0'} bg-gray-800 text-white transition-all duration-300 overflow-hidden flex flex-col`}>
        <div className="p-4 overflow-y-auto flex-1">
          ...
          <nav className="space-y-2">
            ...
            <a href="/configuracion/importar/productos" className="block px-4 py-2 rounded bg-gray-700">Importar Productos</a>
            <a href="/configuracion/importar/clientes" className="block px-4 py-2 rounded hover:bg-gray-700">Importar Clientes</a>
          </nav>
        </div>
        <button
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          {!showPreview ? (
            <div className="bg-white rounded-lg shadow p-6">
              <div className="mb-8">
                ...
                <ol className="list-decimal list-inside space-y-2 text-gray-700">
                  ...
                  <li>Rellena los datos de los productos en el archivo descargado</li>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Se encontraron {preview.length} productos para importar. Por favor revisa los datos antes de confirmar.
              </p>
